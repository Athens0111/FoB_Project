...
$(document).ready(function () {
  ...
  web3.eth.getAccounts().then(function (accounts) {
            ...
            .then(function (productDetails) {
              ...
              if (productDetails[4]) {
                var row =
                  ...
                  "</td><td>" +
                  productDetails[2] +
                  '</td><td><button type="button" class="btn btn-secondary btn-sm" onclick="priceUpdate(' +
                  productDetails[0] +
                  ')">Change price</button></td></tr>';
    ...
    contract.methods
      .getMyTotalOrder(account)
      .call()
      .then(function (totalOrder) {
    ...
    contract.methods
      ...
      .then(function (totalOrder) {
        ...
        for (index = 1; index <= totalOrder; index++) {
          contract.methods
            .getMyNextOrderById(account, index)
            .call()
            .then(function (orderDetails) {
              ...
              if (orderDetails[6]) {
                var row =
                  ...
                  "</td><td>" +
                  orderDetails[4] +
                  '</td><td><button type="button" class="btn btn-primary btn-sm" onclick="delivered(' +
                  orderDetails[0] +
                  ')">Delivered</button>\n<button type="button" class="btn btn-secondary btn-sm" onclick="reject(' +
                  orderDetails[0] +
                  ')">Reject</button></td></tr>';
    ...
    web3.eth
      ...
      .then(function (accounts) {
        ...
        return contract.methods
          .addProduct(pname, pprice, pquantity)
          .send({ from: account });
      })
      .then(function (trx) {
        console.log(trx);
